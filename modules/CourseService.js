...
var CourseService = function (userinfo) {
    this.getStudentsOfCourseByCourseId = function (course_id, callback) {
        var tableData = [];
        ConnPool.doTrans(function (con) {attendDao.getAttendsByCourseId(con,course_id, function (attends) {
                console.log(attends)
            console.log(attends)attends.forEach(function (attend, idx) {
                var tmp = {
                    'student_id': null,
                    'student_name': null,
                    'score': null,
                ...
                asStudentDao.getAsStudentByStudentId(con,attend['student_id'], function (as_student) {
                    ...
                    console.log(as_student)userDao.getUserByUserId(con,as_student[0]['user_id'], function (user) {
                        console.log(user)tmp.student_name = user[0]['user_name'];
                        tableData.push(tmp);
                        if (tableData.length === attends.length) {con.commit(function () {
                            callback(tableData);
                        })
                    }
                })
